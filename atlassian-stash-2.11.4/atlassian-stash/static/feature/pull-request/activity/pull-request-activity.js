define('feature/pull-request/pull-request-activity', [
    'jquery',
    'util/events',
    'underscore',
    'util/ajax',
    ...
    FileContent
        ...
        function PullRequestActivity(contextSelector, pullRequest, fromType, fromId, options) {
            ...
            this.fromId = fromId;
            this.dataLoadedEvent = 'stash.feature.pullRequestActivity.dataLoaded';
        ...
        PullRequestActivity.prototype.init = function(options) {
            ...
            if (!this.loadedRange.isEmpty()) { // render the initial diffs now since attachNewContent won't be called.
                this.renderedDiffFileContents = this.renderedDiffFileContents.concat(
                    PullRequestActivity.renderDiffs(
                        this._$container.children('.diff-comment-activity'),
                        options.diffCommentData,
                        new CommitRange({
            ...
            events.on('stash.widget.approve-button.added', this._approvalHandler);
            events.on('stash.widget.approve-button.removed', this._approvalHandler);
            events.on('stash.feature.comments.lastCommentDeleted', lastCommentDeletedHandler);
            events.on('stash.keyboard.shortcuts.pullrequest.addCommentHandler', bindAddGeneralCommentHandler);
        };

        PullRequestActivity.prototype.reset = function() {
            events.off('stash.widget.approve-button.added', this._approvalHandler);
            events.off('stash.widget.approve-button.removed', this._approvalHandler);
            events.off('stash.feature.comments.lastCommentDeleted', lastCommentDeletedHandler);
            events.off('stash.keyboard.shortcuts.pullrequest.addCommentHandler', bindAddGeneralCommentHandler);
        ...
        PullRequestActivity.prototype.checkCommentIsNew = function(comment) {
            comment.isUnread =
                comment.updatedDate > this.lastViewed &&
                    (!pageState.getCurrentUser() || comment.author.name !== pageState.getCurrentUser().getName());
        ...
        PullRequestActivity.prototype.requestData = function(start, limit) {
            ...
            var permalinkParams = start === 0 && !_.isUndefined(this.fromType) && !_.isUndefined(this.fromId) ?
                    { fromType: this.fromType, fromId: this.fromId } : {};

            this._$spinner.spin('large');
            return ajaxUtil.rest({
                url : navbuilder.rest()
                      .project(this.pullRequestPathComponents.projectKey)
                      .repo(this.pullRequestPathComponents.repoSlug)
                      .pullRequest(this.pullRequestPathComponents.pullRequestId)
                      .activities()
                      .withParams($.extend(permalinkParams, {
                          start : start,
                          limit : limit,
                ...
                statusCode : {
                    ...
                    '404' :  function(xhr, textStatus, errorThrown, errors, dominantError) {
                        var title =
                            self.fromType === 'activity' ? stash_i18n("stash.web.pull-request.activity.notfound.title", "Activity not found")
                                                         : stash_i18n("stash.web.pull-request.comment.notfound.title", "Comment not found");
                        var message =
                            self.fromType === 'activity' ? stash_i18n("stash.web.pull-request.activity.notfound.message", "The requested activity does not exist.")
                                                         : stash_i18n("stash.web.pull-request.comment.notfound.message", "The requested comment does not exist. It may have been deleted.");
                        return $.extend({}, dominantError, {
                            title: title,
                            titleClass: 'confirm-header',
                            message: message,
                            fallbackTitle: stash_i18n("stash.web.pull-request.activity.notfound.fallback.title", "Ok"),
                            fallbackUrl: navbuilder.project(self.pullRequestPathComponents.projectKey)
                                            .repo(self.pullRequestPathComponents.repoSlug)
                                            .pullRequest(self.pullRequestPathComponents.pullRequestId)
                                            .overview().build(),
                            canClose: false,
                            shouldReload: false
                        });
                ...
                if (!self.currentTime) {
                    ...
                    var currentTime = new Date(xhr.getResponseHeader('Date')).getTime(),
                        lastViewedKey = clientStorage.buildKey('last-viewed', 'pull-request');
        ...
        PullRequestActivity.prototype.decorateForFocus = function(data) {
            ...
            if (isActivityPermalink) {
                ...
                var focusComment = function (comment) {
        ...
        PullRequestActivity.prototype.onAttachFirstPermalinkPage = function(data, attachmentMethod) {
            ...
            var $loadPrevious = $(stash.feature.pullRequest.loadPreviousActivities());
            ...
            function loadPreviousActivities() {
                ...
                ajaxUtil.rest({
                    url : navbuilder.rest()
                        .project(self.pullRequestPathComponents.projectKey)
                        .repo(self.pullRequestPathComponents.repoSlug)
                        .pullRequest(self.pullRequestPathComponents.pullRequestId)
                        .activities()
                        .withParams($.extend(params, {
                            avatarSize: stash.widget.avatarSizeInPx({ size: 'medium' }),
                            markup: true
                        }))
                        .build()
                    ...
                    self.attachActivities(nonDuplicates, function(element) { $loadPrevious.after(element); });
        ...
        PullRequestActivity.prototype.attachActivities = function(values, attach) {
            ...
            var $newItems = $(_.map(values, function(activity) {
                var activityItem = stash.feature.pullRequest.activityListItem({
                    activity : activity,
            ...
            this.fileCommentContexts = PullRequestActivity.addBreadcrumbsAndBindFileComments(
                $newItems.filter('.file-comment-activity'),
                dataByActivityId,
                commitRange
            );

            var diffs = PullRequestActivity.renderDiffs(
                $newItems.filter('.diff-comment-activity'),
                dataByActivityId,
                commitRange
            );
        ...
        PullRequestActivity.prototype.attachNewContent = function(data, attachmentMethod) {
            ...
            var inlineInfos = this.attachActivities(data.values,
                function(element) { self.attachContent(attachmentMethod, element); });    //attach fn
            ...
            function scrollToFocused($root) {
                if (!self._inited) { // destroyed in the meantime.
                    return;
                }
                ...
                if (isCommentPermalink) {
                    ...
                    events.once('stash.feature.comments.commentContainerAdded', scrollToFocused);
                }
            }
        ...
        PullRequestActivity.renderDiffForComment = function($container, data, commitRange) {
            ...
            var initPromise = fileContent.init(fileChange, undefined, undefined, {
                ...
                // all attributes are returned as arrays of length 1
                pullRequestDiffCurrent: isCurrent,
                pullRequestDiffLinkUrl: navbuilder.currentPullRequest()
                                    .diff()
                                    .change(fileChange.getPath())
                                    .build(),
                toolbarWebFragmentLocationPrimary : 'stash.pull-request.activity.diff.toolbar.primary',
                toolbarWebFragmentLocationSecondary : 'stash.pull-request.activity.diff.toolbar.secondary'
            });
        ...
        PullRequestActivity.addBreadcrumbsAndBindFileComments = function($fileCommentActivities, dataByActivityId, commitRange) {
            ...
            $fileCommentActivities.each(function() {
                ...
                $el.find('.breadcrumbs').append(
                    stash.widget.breadcrumbs.crumbs({
                        pathComponents: components,
                        primaryLink: isCurrent ? navbuilder.currentPullRequest().diff().change(path).build() : undefined
                    })
                );
        ...
        PullRequestActivity.prototype.addApprovalActivity = function (approved, user, date) {
            var $firstItem = $("#pull-request-activity .comment-form-container");
            $(stash.feature.pullRequest.activityApprovalListItem({
                user: user,
                action: approved ? 'APPROVED' : 'UNAPPROVED',
                activityDate: date
            })).hide().insertAfter($firstItem).fadeIn('slow');
        };
