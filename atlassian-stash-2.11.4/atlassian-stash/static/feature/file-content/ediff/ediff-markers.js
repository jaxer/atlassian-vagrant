define('feature/file-content/ediff/ediff-markers', [
    'bacon',
    'jquery',
    'underscore',
    'util/bacon',
 ...
 *
 * @exports feature/file-content/ediff/ediff-markers
 */
function (
    Bacon,
    $,
    ...
    // Ediff is really slow in IE8, so we disable it in that case.
    var isEdiffDisabled = $.browser.msie && parseInt($.browser.version, 10) < 9;
    ...
    function addEdiffMarkers(diffView, change) {
        ...
        var lines = Bacon.fromBinder(function(sink) {
            change.eachLine(sink).done(function() {
                sink(new Bacon.End());
            });
            return $.noop;
        });
    ...
    function init(options) {
        ...
        options.diffView.on('change', changeHandler);
    ...
    function destroy(diffView) {
        diffView.off('change', changeHandler);
    }
