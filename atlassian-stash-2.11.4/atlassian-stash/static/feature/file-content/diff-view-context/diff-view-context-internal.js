 ...
 */
define('feature/file-content/diff-view-context/diff-view-context-internal',[
    'jquery',
    'underscore',
    'util/function',
    ...
    exports.fetchContext = function fetchContext(startIndex, endIndex, browse, maxContext, options) {
        ...
        function sliceLines(data, start, end) {
            return data && _.extend({}, data, {
                lines: end == null ?
                       data.lines.slice(start) : // IE8 can't handle an undefined second parameter.
         ...
         */
        var getExactResults = function (callback) {
            return $.when(
                    // Fetch 1 more result to work out if we're at the end
                    startIndex === 0 ? null : callback(startIndex, maxContext + (!endIndex ? 1 : 0)),
                    !endIndex ? null : callback(startIndex + limit - maxContext, maxContext)
                ).then(function (lines1, lines2) {
                    if (!lines2) {
            ...
            return browse(startIndex, limit).then(function (data) {
                ...
                if (limit <= maxContext * 2) {
                    ...
                    return getExactResults(function (start) {
                        ...
                        return _.extend({}, result, {contexts: [
                            sliceLines(result.contexts[0], 0, maxContext),
                            sliceLines(result.contexts[1], -maxContext)]
                        });
                    });
                }
            });
