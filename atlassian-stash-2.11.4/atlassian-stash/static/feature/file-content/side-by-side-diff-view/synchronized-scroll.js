define('feature/file-content/side-by-side-diff-view/synchronized-scroll', [
    'jquery',
    'underscore',
    'util/events',
    'util/function',
    'util/math',
    'util/synchronized-scroll',
 ...
 * side-by-side diffs.
 *
 * @exports feature/file-content/side-by-side-diff-view/synchronized-scroll
 */
function(
    $,
    _,
    ...
    CodeMirrorRegion.prototype._getCachedInfo = function() {
        if (!this._cachedInfo) {
            this._cachedInfo = {
                height : this._numLines ? // return 0 quickly if this is a 'filler' region with no lines in it.
                        this._editor.heightAtLine(this._endIndex, 'local') - this._editor.heightAtLine(this._startIndex, 'local') :
                        0,
                offsetTop : this._editor.heightAtLine(Math.max(0, this._startIndex), 'local')
            };
        }
    ...
    function getCombinedRegion(linkedRegions, getOffset) {
        var height, offset, scrollable;
        return {
            _id : 'combined_' + _.pluck(linkedRegions, '_id').join('_'),
            _setCombinedScrollable : function(s) {
                scrollable = s;
            },
            getHeight : function() {
                if (height == null) {
    ...
    function UnbackedScrollable(id, options) {
        ...
        function setScrollTop(y) {
            scrollTop = math.clamp(0, options.getScrollHeight())(y);
        }
         ...
         * @private
         */
        this._scrollToNative = function(x, y) { // record a user (non-programmatic) scroll
            if (y != null) {
                setScrollTop(y);
                if (scrollHandler) {
                    scrollHandler();
        ...
        /**
         * Set a function to be called on a 'native' scroll (e.g., _scrollToNative).
    ...
    function getRegionsFromLineInfos(diffView, lineInfos, fromEditor, toEditor, options) {
        ...
        function createRegion(editor, range, segment, lineInfos, idPrefix) {
            return new CodeMirrorRegion(
                editor,
                range.start,
                range.end,
                idPrefix + '_' + range.start + '_' + range.end,
                segment,
                lineInfos);
        }
        ...
        function addLinkedRegions(fromSegmentInfo, toSegmentInfo) {
            var fromRegion = createRegion(
                fromEditor,
                fromSegmentInfo.from,
                fromSegmentInfo.segment,
                fromSegmentInfo.from.lineInfos,
                'from');
            var toRegion = createRegion(
                toEditor,
                toSegmentInfo.to,
                toSegmentInfo.segment,
                toSegmentInfo.to.lineInfos,
                'to');
         ...
         */
        var getOffsetAtIndex = _.compose(
            fn.partialRight(_.reduce, math.add, 0), // = sum of
        ...
        var boundEvents = events.chain().on('stash.feature.changeset.difftree.collapseAnimationFinished', invalidateAll);
     ...
     */
    function getSynchronizedScrollHandlers(diffView, fromScrollable, toScrollable, combinedScrollable, regionData) {
        ...
        function getLinkedRegion(region, scrollable) {
            ...
            return _.find(linkedRegions, _.compose(fn.eq(scrollable._editor), fn.dot('_editor')));
        }
        ...
        var handlers = {
            fromHandler : getHandler(fromScrollable, _.compact([ toScrollable, combinedScrollable ]), {
                includeHorizontal : true
            }),
            toHandler : getHandler(toScrollable, _.compact([ fromScrollable, combinedScrollable ]), {
                includeHorizontal : true
            })
        };
        if (combinedScrollable) {
    ...
    function setupScrolling(diffView, lineInfos, fromEditor, toEditor, options) {
        ...
        var synchronizedScrollHandlers = getSynchronizedScrollHandlers(diffView, fromScrollable, toScrollable, combinedScrollable, regions);
        fromEditor.on('scroll', synchronizedScrollHandlers.fromHandler);
        toEditor.on('scroll', synchronizedScrollHandlers.toHandler);

        var synchronizedScrollingInfo = {
            linkedFromAndToRegions: regions.linkedFromAndToRegions,
            destroy : function() {
                fromEditor.off('scroll', synchronizedScrollHandlers.fromHandler);
                toEditor.off('scroll', synchronizedScrollHandlers.toHandler);
                regions.destroy();
            }
        };

        if (combinedScrollable) {
            combinedScrollable._setScrollHandler(synchronizedScrollHandlers.combinedHandler);
