define('feature/permission/permission-table', [
    'jquery',
    'underscore',
    'util/ajax',
    'util/function',
    ...
    'model/page-state',
    'feature/permission/multi-selector',
    'exports'
], function(
    $,
    _,
    ...
    function makeDataSource(navBuilder, entityType) {

        function getNonMembers(params) {

            return ajax.rest({
                url : navBuilder[entityType + 's']().none().build(),
                type : 'GET',
                data : params
            });
        ...
        function getMembers(params) {
            return ajax.rest({
                url : navBuilder[entityType + 's']().build(),
                type : 'GET',
                data : params
            });
        ...
        function setPermission(permission, ids) {
            return ajax.rest({
                url : navBuilder[entityType + 's']().withParams({permission: permission, name : ids }).build(),
                type : 'PUT',
                statusCode : {
                    '400' : false,
                    '403' : false,
        ...
        function removePermissions(id) {
            return ajax.rest({
                url : navBuilder[entityType + 's']().withParams({ name : id }).build(),
                type : 'DELETE',
                statusCode : {
                    '400' : false,
                    '403' : false,
    ...
    function PermissionTable(container, permissionsNavBuilder, permissionsOrderedList, userOrGroup, currentUserHighestPerm, opts) {
        ...
        if (this._entityType === 'user') {
            ...
            this._rowTemplate = stash.feature.permission.userPermissionRow;
            ...
            this._rowTemplate = stash.feature.permission.groupPermissionRow;
    ...
    PermissionTable.text = {
        entityList : {
            buttonLoadMoreText: stash_i18n('stash.web.permission.table.load.more', 'Load more')
        },
        pendingModification : stash_i18n('stash.web.pending.request', 'You have made permission changes that have not yet reached Stash. If you leave this page, those changes will be lost.'),
        cantGrantHigherPerms : function(permName) { return stash_i18n('stash.web.permission.table.cantgranthigher.tooltip', 'You cannot grant the {0} permission', permName); },
        inheritedPerm : stash_i18n('stash.web.permission.table.inheritedperm.tooltip', 'This permission is inherited from a higher permission')
    };

    PermissionTable.userText = $.extend(true, {
        inactiveEntity : function(name) {
            return stash_i18n('stash.web.permission.table.inactive.user',
                   '{0} is deleted or marked as inactive in a remote directory', name);
        },
        cantRevokeHigherPerms : function(permName) { return stash_i18n('stash.web.permission.table.cantrevokehigheruser.tooltip', 'You cannot change permissions for a {0} ', permName); },
        cantGrantSelfHigherPerms : function(permName) { return stash_i18n('stash.web.permission.table.cantgrantselfhigher.tooltip', 'You cannot grant yourself the {0} permission', permName); },
        implicitPerm : stash_i18n('stash.web.permission.table.basePerm.tooltip', 'To remove this permission, click the X to the right to remove the entire row.'),
        noMoreItemsText: stash_i18n("stash.web.permission.table.users.none.added", "All users already have permissions set.")
    }, PermissionTable.text);

    PermissionTable.groupText =  $.extend(true, {
        inactiveEntity : function(name) {
            return stash_i18n('stash.web.permission.table.invisible.group', '{0} is deleted', name);
        },
        cantRevokeHigherPerms : function(permName) { return stash_i18n('stash.web.permission.table.cantrevokehighergroup.tooltip', 'You cannot change permissions for a {0} group', permName); },
        implicitPerm : stash_i18n('stash.web.permission.table.basePerm.tooltip', 'To remove this permission remove the group row'),
        noMoreItemsText: stash_i18n("stash.web.permission.table.groups.none.added", "All groups already have permissions set.")
    }, PermissionTable.text);
    ...
    PermissionTable.prototype._getModalItem = function(restElem) {
        return {
            content: $(stash.feature.permission.permissionModalItem({
                entity : restElem,
    ...
    PermissionTable.prototype._initAddMultiSelector = function() {
        ...
        this._multiSelector = new PermissionMultiSelector({
            ...
            add: function(payload) {
                return self._dataSource.setPermission(payload.permission, _.pluck(payload.entities, 'name'))
                    .done(function() {
        ...
        return jqXhr.fail(function(xhr, textStatus, errorThrown, data) {
            if (data && data.errors) {
                ...
                $.each(data.errors, function(i) {
                    $sourceRow.after(stash.feature.permission.errorPermissionRow({ entityId: entityId, numRows : numRows, message: data.errors[i].message }));
                });
            }
        });
    };
    ...
    PermissionTable.prototype._initRows = function() {
        ...
        this._$tbody.on('change', ':checkbox', function() {
            ...
            var $this = $(this),
                ...
                entityId = $parentRow.attr('data-entity-id'),
    ...
    PermissionTable.prototype._loadItems = function() {
        ...
        self._dataSource.getMembers({
            ...
            avatarSize: stash.widget.avatarSizeInPx({ size: 'xsmall' })
                ...
                var addedEntityNames = _.chain(self._addedEntities)
                    .pluck(self._entityType)
                    .pluck('name')
                    .invoke('toLowerCase')
                    .value();

                var values = _.chain(data.values)
                ...
                if (values.length) {
                    self._$tbody.append(
                        _.reduce(values, function(memo, item) {
