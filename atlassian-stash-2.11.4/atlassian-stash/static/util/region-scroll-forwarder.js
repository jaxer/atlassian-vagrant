define('util/region-scroll-forwarder', [
    'bacon',
    'jquery',
    'underscore',
    'util/bacon',
 ...
 *
 * @exports util/region-scroll-forwarder
 */
function(
    Bacon,
    $,
    ...
    function RegionScrollForwarder(scrollProperty, regions) {
        ...
        var forwardingInfo = scrollProperty
            .takeUntil(destroyed)
            .slidingWindow(2,2)
            .map(fn.spread(function(previousScrollInfo, scrollInfo) {
                var offset = 0;
                var prevRegion;
        ...
        function scrollRegionsToEdge(regions, getEdge) {
            _.chain(regions)
                .filter(function(region, i, regions) { // unique by groupId, ensuring first region is chosen
        ...
        var stopForwarding = forwardingInfo
            .onValue(function(forwardingInfo) {
                ...
                if (forwardToRegion !== forwardingInfo.forwardTo) {
                    ...
                    scrollRegionsToEdge(
                        scrollingDown ? betweens.reverse() : betweens,
                        fn.constant(scrollingDown ? 'bottom' : 'top'));
                }
        ...
        function resetPositions() {
            ...
            if (latestForwardingInfo) {
                ...
                scrollRegionsToEdge(_.reject(regions, function(region) {
                    return region === latestForwardingInfo.forwardTo ||
                           region.groupId === latestForwardingInfo.forwardTo.groupId;
                }), function(region) {
                    return _.indexOf(regions, region) < regionIndex ? 'bottom' : 'top';
                });
                latestForwardingInfo.forwardTo.setScrollTop(latestForwardingInfo.localTop);
            }
        }
