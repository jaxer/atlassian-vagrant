define('widget/overflowing-list', [
    'jquery'
], function (
    $
    ) {
    ...
    OverflowingList.defaults = {
        ...
        getItemHtml : function(data, isOverflowed) {
            return '<span>'+ (typeof data === 'string' ? data : JSON.stringify(data)) + '</span>';
        },
        compareFn : function(a, b) { return -1; }
    };
    ...
    function get$OverflowList($container, overflowMenuId) {
        return overflowMenuId ?
               $('#' + overflowMenuId + ' > ul') :
               $container.children('.aui-dropdown2:last > ul');
    }

    function create$OverflowList($container, overflowMenuClass, overflowMenuId) {
        var $menu = $(widget.aui.dropdown2({
            menuId : overflowMenuId,
            menuClass : overflowMenuClass,
            triggerContent : '<div class="extras-dropdown-trigger">' +
                                  aui.badges.badge({'text': '?'}) +
                              '</div>',
            menuContent : '<ul class="aui-list-truncate"></ul>',
            hasCustomStyle : true
        })).appendTo($container);
    ...
    OverflowingList.prototype.addItem = function(item) {
        ...
        for (var i = 0, len = this._items.length; i < len; i++) {
            ...
            if (compareFn(item, this._items[i]) < 0) {
                if (i >= maxOpen) { // goes in the overflow section
                    $item = $(getItemHtml(item, true));
                    $item.wrap('<li/>').parent().insertBefore($items.eq(i).parent());
                    setOverflowCount($container, this._items.length + 1 - maxOpen);
                } else { // goes in the visible section
                    $item = $(getItemHtml(item, false));
                    $item.insertBefore($items.eq(i)).hide().fadeIn('fast');
                    if (this._items.length >= maxOpen) { // we pushed another item over the limit into the overflow.
                        $items.eq(maxOpen - 1).remove();
                        ($overflowList.length ? $overflowList : create$OverflowList($container, overflowMenuClass, overflowMenuId))
                            .prepend($('<li/>').append(getItemHtml(this._items[maxOpen - 1], true)));
                        setOverflowCount($container, this._items.length + 1 - maxOpen);
                    }
                }
        ...
        if (!inserted) { // it goes at the end
            if (this._items.length >= maxOpen) {
                ($overflowList.length ? $overflowList : create$OverflowList($container, overflowMenuClass, overflowMenuId))
                    .append($(getItemHtml(item, true)).wrap('<li/>').parent());
                setOverflowCount($container, this._items.length + 1 - maxOpen);
            } else {
                $item = $(getItemHtml(item, false));
