define('widget/image-tools/image-upload-and-crop', [
    'jquery',
    'underscore',
    'widget/image-tools/image-explorer',
    'widget/image-tools/client-file-reader',
    'widget/image-tools/drag-drop-file-target',
    'widget/image-tools/client-file-iframe-uploader',
    'widget/image-tools/upload-interceptor',
    'widget/image-tools/canvas-cropper',
    'util/text'
], function(
    $,
    _,
    ...
    ImageUploadAndCrop.prototype.defaults = {
        ...
        dragDropUploadPrompt: stash_i18n('stash.web.drag.drop.image.prompt', 'Drag and drop image here'),
        onImageUpload: $.noop,
            ...
            if (!isNaN(this.options.maxImageDimension)) {
                ...
                validatePromise
                    ...
                    .fail(_.bind(function(imageWidth, imageHeight){
                        this._onFileError(stash_i18n('stash.web.avatar.error.file.resolution', 'The selected image size is {0}px * {1}px. The maximum allowed image size is {2}px * {2}px',
                            imageWidth, imageHeight, this.options.maxImageDimension));
                    }, this));
            } else {
    ...
    ImageUploadAndCrop.prototype._onFilesError = function(invalidFiles) {
        ...
        if (invalidFiles && invalidFiles.bySize && invalidFiles.bySize.length){
            ...
            this._onFileError(AJS.escapeHtml(stash_i18n('stash.stash.web.avatar.error.file.too.big', 'File "{0}" is {1} which is larger than the maximum allowed size of {2}',
                                TextUtil.abbreviateText(file.name, 50), TextUtil.formatSizeInBytes(file.size), TextUtil.formatSizeInBytes(this.options.fileSizeLimit))));
        } else {
    ...
    ImageUploadAndCrop.prototype._onFileError = function(error){
        var title = stash_i18n('stash.web.avatar.error.adding.file.title', 'There was an error uploading your image'),
            contents = error || stash_i18n('stash.web.avatar.error.adding.file.contents', 'Please check that your file is a valid image and try again.');
